...
module bkf_core #(
    ...
    input  wire [3*`FX_Q8_16_WIDTH-1:0]             cfg_state_flat,
    input  wire [9*`FX_Q8_16_WIDTH-1:0]            cfg_cov_flat,

    input  wire                    model_valid,
    output wire                    model_ready,
    input  wire [9*`FX_Q8_16_WIDTH-1:0]            f_input_flat,

    output wire                    threshold_valid,
    input  wire                    threshold_ready,
    output wire [3*`FX_Q8_16_WIDTH-1:0]             threshold_flat,
    ...
    input  wire [3*`FX_Q8_16_WIDTH-1:0]             measurement_flat,

    output wire                    result_valid,
    input  wire                    result_ready,
    output wire [3*`FX_Q8_16_WIDTH-1:0]             state_out_flat,
    output wire [9*`FX_Q8_16_WIDTH-1:0]            cov_out_flat,
    ...
    output wire [9*`FX_Q8_16_WIDTH-1:0]            debug_cov_predict_flat,
    output wire [9*`FX_Q8_16_WIDTH-1:0]            debug_sign_cov_flat,
    ...
    localparam signed [`FX_Q8_16_WIDTH-1:0] FX_ONE     = `FX_Q8_16_ONE;
    localparam signed [`FX_Q8_16_WIDTH-1:0] FX_NEG_ONE = -`FX_Q8_16_ONE;
    localparam signed [`FX_Q8_16_WIDTH-1:0] Q_DIAG     = `FX_Q8_16_Q_DIAG;      // 0.001
    localparam signed [`FX_Q8_16_WIDTH-1:0] R_DIAG     = `FX_Q8_16_R_DIAG;    // 0.1
    localparam signed [`FX_Q8_16_WIDTH-1:0] ALPHA      = `FX_Q8_16_ALPHA;   // \sqrt{2/\pi}
    localparam signed [`FX_Q8_16_WIDTH-1:0] FX_MAX     = `FX_Q8_16_MAX;
    localparam signed [`FX_Q8_16_WIDTH-1:0] FX_MIN     = `FX_Q8_16_MIN;
    ...
    reg signed [`FX_Q8_16_WIDTH-1:0] state_post [0:2];
    reg signed [`FX_Q8_16_WIDTH-1:0] cov_post [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] f_matrix [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] state_predict [0:2];
    reg signed [`FX_Q8_16_WIDTH-1:0] measurement_hold [0:2];
    reg signed [`FX_Q8_16_WIDTH-1:0] matrix_temp [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] cov_predict [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] measurement_cov [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] diag_rsqrt [0:2];
    reg signed [`FX_Q8_16_WIDTH-1:0] normalized_cov [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] sign_cov_pre [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] sign_cov [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] self_normalized [0:2];
    reg signed [`FX_Q8_16_WIDTH-1:0] b_diag [0:2];
    reg signed [`FX_Q8_16_WIDTH-1:0] cofactor [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] sign_cov_inverse [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] gain_inner [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] gain [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] observation_q [0:2];
    reg signed [`FX_Q8_16_WIDTH-1:0] state_correction [0:2];
    reg signed [`FX_Q8_16_WIDTH-1:0] cov_update_inner [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] cov_correction [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] determinant;
    reg signed [`FX_Q8_16_WIDTH-1:0] scalar_temp;
    reg signed [`FX_Q8_16_MAC_WIDTH-1:0] mac_accumulator;
    ...
    reg signed [`FX_Q8_16_MAC_WIDTH-1:0]      cov_predict_mac_accumulator_reg;
    ...
    reg signed [`FX_Q8_16_WIDTH-1:0]      add_q_operand_reg;
    ...
    reg signed [`FX_Q8_16_WIDTH-1:0]      cov_sym_left_reg;
    reg signed [`FX_Q8_16_WIDTH-1:0]      cov_sym_right_reg;
    reg [8:0]              cov_sym_write_mask_reg;
    reg [3:0]              cov_sym_next_index_reg;
    reg                    cov_sym_last_reg;

    reg                    measurement_operand_valid;
    reg signed [`FX_Q8_16_WIDTH-1:0]      measurement_operand_reg;
    ...
    reg signed [`FX_Q8_16_WIDTH-1:0]      measurement_sym_left_reg;
    reg signed [`FX_Q8_16_WIDTH-1:0]      measurement_sym_right_reg;
    ...
    reg signed [`FX_Q8_16_WIDTH-1:0]      sign_sym_left_reg;
    reg signed [`FX_Q8_16_WIDTH-1:0]      sign_sym_right_reg;
    ...
    reg signed [`FX_Q8_16_WIDTH-1:0]      post_sym_left_reg;
    reg signed [`FX_Q8_16_WIDTH-1:0]      post_sym_right_reg;
    reg [8:0]              post_sym_write_mask_reg;
    reg [3:0]              post_sym_next_index_reg;
    reg                    post_sym_last_reg;

    reg signed [`FX_Q8_16_WIDTH-1:0] mul_a;
    reg signed [`FX_Q8_16_WIDTH-1:0] mul_b;
    reg signed [`FX_Q8_16_WIDTH-1:0] cofactor_a1;
    reg signed [`FX_Q8_16_WIDTH-1:0] cofactor_b1;
    reg signed [`FX_Q8_16_WIDTH-1:0] cofactor_a2;
    reg signed [`FX_Q8_16_WIDTH-1:0] cofactor_b2;
    reg signed [`FX_Q8_16_WIDTH-1:0] cofactor_transposed;
    wire               mul_request_valid;
    wire               mul_request_ready;
    wire               pipe_result_valid;
    wire               pipe_result_ready;
    wire signed [`FX_Q8_16_WIDTH-1:0] pipe_rounded_product;
    wire signed [`FX_Q8_16_MAC_WIDTH-1:0] pipe_mac_sum;
    wire signed [`FX_Q8_16_WIDTH-1:0] pipe_rounded_mac;
    ...
    wire               rsqrt_en;
    reg signed [`FX_Q8_16_WIDTH-1:0]  rsqrt_x;
    wire signed [`FX_Q8_16_WIDTH-1:0] rsqrt_y;
    wire               rsqrt_valid;
    wire               rsqrt_domain_error;

    wire               asin_en;
    reg signed [`FX_Q8_16_WIDTH-1:0]  asin_x;
    wire signed [`FX_Q8_16_WIDTH-1:0] asin_y;
    wire               asin_valid;
    ...
    wire               divider_busy;
    wire               divider_valid;
    wire signed [`FX_Q8_16_WIDTH-1:0] divider_quotient;
    ...
    wire signed [`FX_Q8_16_WIDTH-1:0] det_finalize_value;
    ...
    function signed [`FX_Q8_16_WIDTH-1:0] branch_average_q16;
        input signed [15:0] sum;
        reg signed [16+`FX_Q8_16_FRAC-1:0] scaled;
        begin
            scaled = {{`FX_Q8_16_FRAC{sum[15]}}, sum};
            if (NUM_BRANCHES == 1)
                branch_average_q16 = (scaled <<< `FX_Q8_16_FRAC);
            else if (NUM_BRANCHES == 8)
                branch_average_q16 = (scaled <<< (`FX_Q8_16_FRAC-3));
            else
                branch_average_q16 = {`FX_Q8_16_WIDTH{1'b0}};
        end
    endfunction

    function signed [`FX_Q8_16_WIDTH-1:0] reduce_self_cov_l8;
        input signed [`FX_Q8_16_WIDTH-1:0] cross_branch_cov;
        reg signed [`FX_Q8_16_WIDTH+7:0] wide_sum;
        reg signed [`FX_Q8_16_WIDTH+7:0] rounded;
        begin
            wide_sum = $signed(`FX_Q8_16_ONE) + ({{8{cross_branch_cov[`FX_Q8_16_WIDTH-1]}}, cross_branch_cov} <<< 3)
                       - {{8{cross_branch_cov[`FX_Q8_16_WIDTH-1]}}, cross_branch_cov};
            if (wide_sum < 0)
                rounded = -(((-wide_sum) + $signed({{(`FX_Q8_16_WIDTH+5){1'b0}}, 3'b100})) >>> 3);
            else
                rounded = (wide_sum + $signed({{(`FX_Q8_16_WIDTH+5){1'b0}}, 3'b100})) >>> 3;
            reduce_self_cov_l8 = rounded[`FX_Q8_16_WIDTH-1:0];
        end
    endfunction
    ...
    assign debug_reduced_observation_flat[`FX_Q8_16_WIDTH-1:0] = observation_q[0];
    assign debug_reduced_observation_flat[2*`FX_Q8_16_WIDTH-1:`FX_Q8_16_WIDTH] = observation_q[1];
    assign debug_reduced_observation_flat[3*`FX_Q8_16_WIDTH-1:2*`FX_Q8_16_WIDTH] = observation_q[2];
    ...
    generate
        for (state_gen = 0; state_gen < 3; state_gen = state_gen + 1) begin : PACK_STATE
            assign threshold_flat[(state_gen*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH] = state_predict[state_gen];
            assign state_out_flat[(state_gen*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH] = state_post[state_gen];
        end
    endgenerate
    ...
    generate
        for (matrix_gen = 0; matrix_gen < 9; matrix_gen = matrix_gen + 1) begin : PACK_MATRIX
            assign cov_out_flat[(matrix_gen*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH] = cov_post[matrix_gen];
            assign debug_cov_predict_flat[(matrix_gen*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH] = cov_predict[matrix_gen];
            assign debug_sign_cov_flat[(matrix_gen*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH] = sign_cov[matrix_gen];
            assign debug_gain_flat[(matrix_gen*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH] = gain[matrix_gen];
        end
    endgenerate
    ...
    always @* begin
        cofactor_a1 = {`FX_Q8_16_WIDTH{1'b0}};
        cofactor_b1 = {`FX_Q8_16_WIDTH{1'b0}};
        cofactor_a2 = {`FX_Q8_16_WIDTH{1'b0}};
        cofactor_b2 = {`FX_Q8_16_WIDTH{1'b0}};
        case (element_index)
    ...
    always @* begin
        mul_a = {`FX_Q8_16_WIDTH{1'b0}};
        mul_b = {`FX_Q8_16_WIDTH{1'b0}};
        case (state)
            ST_XPRED: begin
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            dot_index <= 2'd0;
            element_index <= 4'd0;
            determinant <= {`FX_Q8_16_WIDTH{1'b0}};
            scalar_temp <= {`FX_Q8_16_WIDTH{1'b0}};
            mac_accumulator <= {`FX_Q8_16_MAC_WIDTH{1'b0}};
            overflow_flag <= 1'b0;
            numeric_error <= 1'b0;
            solver_error <= 1'b0;
            ...
            cov_predict_mac_accumulator_reg <= {`FX_Q8_16_MAC_WIDTH{1'b0}};
            ...
            measurement_sym_left_reg <= {`FX_Q8_16_WIDTH{1'b0}};
            measurement_sym_right_reg <= {`FX_Q8_16_WIDTH{1'b0}};
            ...
            sign_sym_left_reg <= {`FX_Q8_16_WIDTH{1'b0}};
            sign_sym_right_reg <= {`FX_Q8_16_WIDTH{1'b0}};
            ...
            for (reset_index = 0; reset_index < 3; reset_index = reset_index + 1) begin
                state_post[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                state_predict[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                measurement_hold[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                diag_rsqrt[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                b_diag[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                self_normalized[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                observation_q[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                state_correction[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                branch_sum_hold[reset_index] <= 16'sd0;
            end
            for (reset_index = 0; reset_index < 9; reset_index = reset_index + 1) begin
                cov_post[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                f_matrix[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                matrix_temp[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                cov_predict[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                measurement_cov[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                normalized_cov[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                sign_cov_pre[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                sign_cov[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                cofactor[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                sign_cov_inverse[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                gain_inner[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                gain[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                cov_update_inner[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                cov_correction[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
            end
        end else begin
            cov_predict_overflow_local_valid_reg <= 1'b0;
                ...
                ST_IDLE: begin
                    if (cfg_valid) begin
                        for (load_index = 0; load_index < 3; load_index = load_index + 1)
                            state_post[load_index] <= $signed(cfg_state_flat[(load_index*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH]);
                        for (load_index = 0; load_index < 9; load_index = load_index + 1)
                            cov_post[load_index] <= $signed(cfg_cov_flat[(load_index*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH]);
                        configured <= 1'b1;
                        overflow_flag <= 1'b0;
                        numeric_error <= 1'b0;
                        solver_error <= 1'b0;
                        ...
                        for (load_index = 0; load_index < 9; load_index = load_index + 1)
                            f_matrix[load_index] <= $signed(f_input_flat[(load_index*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH]);
                        for (load_index = 0; load_index < 3; load_index = load_index + 1)
                            measurement_hold[load_index] <= $signed(measurement_flat[(load_index*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH]);
                        row_index <= 2'd0;
                        col_index <= 2'd0;
                        dot_index <= 2'd0;
                        mac_accumulator <= {`FX_Q8_16_MAC_WIDTH{1'b0}};
                ...
                ST_COV_INNER, ST_COV_OUTER, ST_GAIN, ST_COV_UP_INNER, ST_COV_CORR: begin
                    if (pipe_result_valid && (pipe_result_operation == state)) begin
                        if (pipe_result_dot_index == 2'd0) begin
                            ...
                            // Preserve the full (2*W+2)-bit dot-product result.  The

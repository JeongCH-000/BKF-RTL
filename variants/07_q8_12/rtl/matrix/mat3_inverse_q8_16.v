...
module mat3_inverse_q8_16 (
    ...
    input  wire                    rst_n,
    input  wire                    start,
    input  wire [9*`FX_Q8_16_WIDTH-1:0]            matrix_flat,
    output wire                    busy,
    output reg                     valid,
    output wire [9*`FX_Q8_16_WIDTH-1:0]            inverse_flat,
    output reg signed [`FX_Q8_16_WIDTH-1:0]       determinant,
    ...
    wire signed [`FX_Q8_16_WIDTH-1:0] matrix [0:8];
    wire signed [`FX_Q8_16_WIDTH-1:0] product_a [0:8];
    wire signed [`FX_Q8_16_WIDTH-1:0] product_b [0:8];
    wire signed [`FX_Q8_16_WIDTH-1:0] cofactor_wire [0:8];
    wire [8:0] product_a_overflow;
    wire [8:0] product_b_overflow;
    wire [8:0] subtract_overflow;
    wire signed [`FX_Q8_16_WIDTH-1:0] determinant_wire;
    wire determinant_overflow;

    reg [1:0] state;
    reg [3:0] element_index;
    reg signed [`FX_Q8_16_WIDTH-1:0] cofactor_hold [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] inverse [0:8];
    reg signed [`FX_Q8_16_WIDTH-1:0] divider_numerator;
    wire divider_start;
    wire divider_busy;
    wire divider_valid;
    ...
    generate
        for (io_index = 0; io_index < 9; io_index = io_index + 1) begin : IO
            assign matrix[io_index] = $signed(matrix_flat[(io_index*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH]);
            assign inverse_flat[(io_index*`FX_Q8_16_WIDTH) +: `FX_Q8_16_WIDTH] = inverse[io_index];
        end
    endgenerate
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            for (reset_index = 0; reset_index < 9; reset_index = reset_index + 1) begin
                cofactor_hold[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
                inverse[reset_index] <= {`FX_Q8_16_WIDTH{1'b0}};
            end
        end else begin
            valid <= 1'b0;
            case (state)
                ST_IDLE: begin
                    if (start) begin
                        ...
                        if (($signed(determinant_wire) < $signed(`FX_Q8_16_DET_FLOOR)) &&
                            ...
                            determinant <= determinant_wire[`FX_Q8_16_WIDTH-1] ? -`FX_Q8_16_DET_FLOOR : `FX_Q8_16_DET_FLOOR;
                            solver_error <= 1'b1;
                        end else begin

...
// Register-separated iterative unsigned-magnitude divider with signed Q8.F
// I/O. Computes round-to-nearest/ties-away((numerator << F) / denominator).
// Iteration count remains (W + 1 + F); saturation uses the signed W-bit range.
module fx_divider_q8_16 (
    input  wire                    clk,
    input  wire                    rst_n,
    input  wire                    start,
    input  wire signed [`FX_Q8_16_WIDTH-1:0]      numerator,
    input  wire signed [`FX_Q8_16_WIDTH-1:0]      denominator,
    output reg                     busy,
    output reg                     valid,
    output reg signed [`FX_Q8_16_WIDTH-1:0]       quotient,

...
// saturated at the existing fixed-point boundary, checked against the strict
...
module fx_determinant_finalize_pipeline (
    ...
    output wire signed [`FX_Q8_16_WIDTH-1:0]      result_determinant,
    ...
    localparam signed [`FX_Q8_16_WIDTH-1:0] DET_FLOOR = `FX_Q8_16_DET_FLOOR;
    ...
    reg signed [`FX_Q8_16_MAC_WIDTH-1:0] capture_accumulator_reg;

    reg signed [`FX_Q8_16_WIDTH-1:0] raw_determinant_reg;
    reg               raw_overflow_reg;

    reg signed [`FX_Q8_16_WIDTH-1:0] floor_determinant_reg;
    reg               floor_near_zero_reg;
    reg               floor_overflow_reg;

    reg signed [`FX_Q8_16_WIDTH-1:0] result_determinant_reg;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            capture_accumulator_reg <= {`FX_Q8_16_MAC_WIDTH{1'b0}};
            raw_determinant_reg <= {`FX_Q8_16_WIDTH{1'b0}};
            raw_overflow_reg <= 1'b0;
            floor_determinant_reg <= {`FX_Q8_16_WIDTH{1'b0}};
            floor_near_zero_reg <= 1'b0;
            floor_overflow_reg <= 1'b0;
            result_determinant_reg <= {`FX_Q8_16_WIDTH{1'b0}};
            ...
            // Strict floor test: only -DET_FLOOR < determinant < DET_FLOOR is clamped.
            // A rounded zero has a clear sign bit and clamps to +DET_FLOOR.
            if (round_valid_reg && !floor_valid_reg) begin
                ...
                if (($signed(raw_determinant_reg) < $signed(DET_FLOOR)) &&
                    ...
                    floor_determinant_reg <= raw_determinant_reg[`FX_Q8_16_WIDTH-1] ?
                                             -DET_FLOOR : DET_FLOOR;
            ...
            // Full signed (2*W+2)-bit accumulator capture; no quantization occurs.

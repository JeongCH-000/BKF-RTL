...
// Stage 1 captures decoded operands/control, stage 2 registers the full 2*W-bit
// product, stage 3 registers the rounded product and full (2*W+2)-bit MAC sum, and
...
module fx_mul_mac_pipeline (
    ...
    input  wire signed [`FX_Q8_16_WIDTH-1:0]      request_operand_a,
    input  wire signed [`FX_Q8_16_WIDTH-1:0]      request_operand_b,
    input  wire signed [`FX_Q8_16_MAC_WIDTH-1:0]      request_accumulator,
    ...
    output wire signed [`FX_Q8_16_WIDTH-1:0]      result_rounded_product,
    output wire signed [`FX_Q8_16_MAC_WIDTH-1:0]      result_mac_sum,
    output wire signed [`FX_Q8_16_WIDTH-1:0]      result_rounded_mac,
    ...
    reg signed [`FX_Q8_16_WIDTH-1:0] stage1_operand_a;
    reg signed [`FX_Q8_16_WIDTH-1:0] stage1_operand_b;
    reg signed [`FX_Q8_16_MAC_WIDTH-1:0] stage1_accumulator;
    ...
    reg signed [`FX_Q8_16_PRODUCT_WIDTH-1:0] stage2_product;
    reg signed [`FX_Q8_16_MAC_WIDTH-1:0] stage2_accumulator;
    ...
    reg signed [`FX_Q8_16_WIDTH-1:0] stage3_rounded_product;
    reg signed [`FX_Q8_16_MAC_WIDTH-1:0] stage3_mac_sum;
    ...
    wire signed [`FX_Q8_16_PRODUCT_WIDTH-1:0] stage1_product;
    wire signed [`FX_Q8_16_MAC_WIDTH-1:0] stage2_product_extended;
    wire signed [`FX_Q8_16_MAC_WIDTH-1:0] stage2_mac_sum;

    `include "rtl/common/fx_q8_16_functions.vh"

    assign stage1_product = $signed(stage1_operand_a) * $signed(stage1_operand_b);
    assign stage2_product_extended = {{2{stage2_product[`FX_Q8_16_PRODUCT_WIDTH-1]}}, stage2_product};
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            stage1_operand_a <= {`FX_Q8_16_WIDTH{1'b0}};
            stage1_operand_b <= {`FX_Q8_16_WIDTH{1'b0}};
            stage1_accumulator <= {`FX_Q8_16_MAC_WIDTH{1'b0}};
            stage1_operation <= 6'd0;
            stage1_element_index <= 4'd0;
            stage1_row_index <= 2'd0;
            stage1_col_index <= 2'd0;
            stage1_dot_index <= 2'd0;
            stage2_product <= {`FX_Q8_16_PRODUCT_WIDTH{1'b0}};
            stage2_accumulator <= {`FX_Q8_16_MAC_WIDTH{1'b0}};
            stage2_operation <= 6'd0;
            stage2_element_index <= 4'd0;
            stage2_row_index <= 2'd0;
            stage2_col_index <= 2'd0;
            stage2_dot_index <= 2'd0;
            stage3_rounded_product <= {`FX_Q8_16_WIDTH{1'b0}};
            stage3_mac_sum <= {`FX_Q8_16_MAC_WIDTH{1'b0}};

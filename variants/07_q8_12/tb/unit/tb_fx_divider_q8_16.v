...
module tb_fx_divider_q8_16;
    ...
    reg rst_n;
    reg start;
    reg signed [W-1:0] numerator;
    reg signed [W-1:0] denominator;
    wire busy;
    wire valid;
    wire signed [W-1:0] quotient;
    wire divide_by_zero;
    wire overflow;
    ...
    reg signed [W-1:0] random_numerator;
    reg signed [W-1:0] random_denominator;
    ...
    function signed [W-1:0] reference_divide;
        input signed [W-1:0] n;
        input signed [W-1:0] d;
        reg signed [W:0] n_ext;
        reg signed [W:0] d_ext;
        reg [W:0] n_mag;
        reg [W:0] d_mag;
        reg [W+F:0] scaled_numerator;
        reg [W+F+1:0] q_mag;
        reg negative;
        begin
            n_ext = {n[W-1], n};
            d_ext = {d[W-1], d};
            n_mag = n[W-1] ? -n_ext : n_ext;
            d_mag = d[W-1] ? -d_ext : d_ext;
            negative = n[W-1] ^ d[W-1];
            if (d_mag == {(W+1){1'b0}}) begin
                reference_divide = n[W-1] ? TB_MIN_WORD : TB_MAX_WORD;
            end else begin
                scaled_numerator = {n_mag, {F{1'b0}}};
                q_mag = (scaled_numerator + (d_mag >> 1)) / d_mag;
                if (!negative && (q_mag > TB_MAX))
                    reference_divide = TB_MAX_WORD;
                else if (negative && (q_mag > (-TB_MIN)))
                    reference_divide = TB_MIN_WORD;
                else if (negative && (q_mag == (-TB_MIN)))
                    reference_divide = TB_MIN_WORD;
                else if (negative)
                    reference_divide = -$signed(q_mag[W-1:0]);
                else
                    reference_divide = q_mag[W-1:0];
            end
        end
    endfunction
    ...
    function reference_overflow;
        ...
        begin
            ...
            if (d_mag == {(W+1){1'b0}}) begin
                ...
                scaled_numerator = {n_mag, {F{1'b0}}};
                q_mag = (scaled_numerator + (d_mag >> 1)) / d_mag;
                reference_overflow = (!negative && (q_mag > TB_MAX)) ||
                                     (negative && (q_mag > (-TB_MIN)));
            end
        end
    endfunction
    ...
    task check_latency;
        ...
        begin
            if (d == 0) begin
                if (observed_latency != 0) fail("divide-by-zero latency must be zero");
                ...
                if (observed_latency != W+F+3) fail("divider latency differs from preserved iteration formula");
                if (normal_latency < 0)
    ...
    task run_case;
        ...
        input signed [W-1:0] d;
        input integer case_type;
        reg signed [W-1:0] expected_quotient;
    ...
    task test_reset_flush;
        integer wait_cycle;
        begin
            numerator = fx_q16(64'sd196608);
            denominator = fx_q16(64'sd131072);
            start = 1'b1;
            @(negedge clk);
            start = 1'b0;
    ...
    task test_start_while_busy;
        reg signed [W-1:0] first_expected;
        integer accept_cycle;
        integer observed_latency;
        begin
            while (busy) @(negedge clk);
            ...
            numerator = -fx_q16(64'sd65536);
            denominator = fx_q16(64'sd65536);
            start = 1'b1;
            @(negedge clk);
            start = 1'b0;
            while (!valid) @(negedge clk);
            ...
            check_latency(fx_q16(64'sd131072), observed_latency);
            completion_count = completion_count + 1;
            record_response(fx_q16(64'sd196608), fx_q16(64'sd131072), 2,
                            accept_cycle, observed_latency);
            protocol_count = protocol_count + 1;
            run_case(-fx_q16(64'sd65536), fx_q16(64'sd65536), 2);
        end
    endtask
    ...
    initial begin
        ...
        run_case(0, fx_q16(64'sd65536), 0);
        run_case(fx_q16(64'sd196608), fx_q16(64'sd131072), 0);
        run_case(fx_q16(64'sd65536), -fx_q16(64'sd65536), 0);
        run_case(-fx_q16(64'sd65536), fx_q16(64'sd65536), 0);
        run_case(-fx_q16(64'sd65536), -fx_q16(64'sd65536), 0);
        run_case(fx_q16(64'sd65536), fx_q16(64'sd65536), 0);
        run_case(TB_MAX_WORD, fx_q16(64'sd65536), 0);
        run_case(TB_MIN_WORD, fx_q16(64'sd65536), 0);
        run_case(1, TB_MAX_WORD, 0);
        run_case(fx_q16(64'sd131072), fx_q16(64'sd65536), 0);
        run_case(fx_q16(64'sd65536), fx_q16(64'sd196608), 0);
        run_case(1, fx_q16(64'sd131072), 0);
        run_case(-1, fx_q16(64'sd131072), 0);
        run_case((TB_ONE+5)/10, (3*TB_ONE+5)/10, 0); // direct quantization of real 0.1 / 0.3
        run_case((TB_MAX_WORD-1), fx_q16(64'sd65536), 0);
        run_case(-TB_MAX_WORD, fx_q16(64'sd65536), 0);
        run_case(TB_MAX_WORD, 1, 0);
        run_case(-TB_MAX_WORD, 1, 0);
        run_case(TB_MIN_WORD, -fx_q16(64'sd65536), 0);
        run_case(1234, 0, 0);
        run_case(-1234, 0, 0);
        run_case(0, 0, 0);
        run_case(TB_MIN_WORD, TB_MIN_WORD, 0);
        run_case(TB_MAX_WORD, TB_MIN_WORD, 0);
        run_case(TB_MIN_WORD, TB_MAX_WORD, 0);
        run_case(1, -fx_q16(64'sd131072), 0);
        run_case(-1, -fx_q16(64'sd131072), 0);

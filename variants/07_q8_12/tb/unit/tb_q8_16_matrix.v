...
module tb_q8_16_matrix;
    ...
    reg clk;
    reg rst_n;
    reg [9*W-1:0] matmul_a;
    reg [9*W-1:0] matmul_b;
    wire [9*W-1:0] matmul_y;
    wire matmul_overflow;
    reg [9*W-1:0] inverse_input;
    reg inverse_start;
    wire inverse_busy;
    wire inverse_valid;
    wire [9*W-1:0] inverse_y;
    wire signed [W-1:0] inverse_determinant;
    wire inverse_solver_error;
    wire inverse_overflow;

    reg [9*W-1:0] matmul_a_mem [0:MATMUL_CASES-1];
    reg [9*W-1:0] matmul_b_mem [0:MATMUL_CASES-1];
    reg [9*W-1:0] matmul_y_mem [0:MATMUL_CASES-1];
    reg [1:0] matmul_flags_mem [0:MATMUL_CASES-1];
    reg [9*W-1:0] inverse_input_mem [0:INVERSE_CASES-1];
    reg [9*W-1:0] inverse_y_mem [0:INVERSE_CASES-1];
    reg [W-1:0] inverse_det_mem [0:INVERSE_CASES-1];
    ...
    initial begin
        ...
        matmul_a = {9*W{1'b0}};
        matmul_b = {9*W{1'b0}};
        inverse_input = {9*W{1'b0}};
        repeat (3) @(negedge clk);
        rst_n = 1'b1;

        for (index = 0; index < MATMUL_CASES; index = index + 1) begin
            if ((^{matmul_a_mem[index], matmul_b_mem[index], matmul_y_mem[index], matmul_flags_mem[index]}) === 1'bx)
                $fatal(1, "FAIL: missing/X/Z matmul vector case=%0d", index);
        ...
        for (index = 0; index < INVERSE_CASES; index = index + 1) begin
            ...
            if ((^{inverse_input_mem[index], inverse_y_mem[index], inverse_det_mem[index], inverse_flags_mem[index]}) === 1'bx)
                $fatal(1, "FAIL: missing/X/Z inverse vector case=%0d", index);

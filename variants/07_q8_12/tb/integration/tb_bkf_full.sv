...
module tb_bkf_full;
    ...
    reg [3*W-1:0] init_state_mem [0:0];
    reg [9*W-1:0] init_cov_mem [0:0];
    reg [9*W-1:0] f_mem [0:MAX_STEPS-1];
    reg [3*W-1:0] measurement_mem [0:MAX_STEPS-1];
    reg [3*W-1:0] threshold_expected_mem [0:MAX_STEPS-1];
    reg [2:0] observation_expected_mem [0:MAX_STEPS-1];
    reg [3*W-1:0] state_expected_mem [0:MAX_STEPS-1];
    reg [9*W-1:0] cov_expected_mem [0:MAX_STEPS-1];
    reg [9*W-1:0] cov_predict_expected_mem [0:MAX_STEPS-1];
    reg [9*W-1:0] sign_cov_expected_mem [0:MAX_STEPS-1];
    reg [9*W-1:0] gain_expected_mem [0:MAX_STEPS-1];
    reg [W-1:0] determinant_expected_mem [0:MAX_STEPS-1];
    ...
    reg signed [W-1:0] measurement_element;
    reg signed [W-1:0] threshold_element;
        ...
        for (step = 0; step < requested_steps; step = step + 1) begin
            ...
            if (threshold_flat !== threshold_expected_mem[step]) begin
                for (channel = 0; channel < 3; channel = channel + 1) begin
                    if ($signed(threshold_flat[(channel*W) +: W]) !==
                        $signed(threshold_expected_mem[step][(channel*W) +: W]))
                        fail_mismatch(step, "threshold", channel,
                            $signed(threshold_expected_mem[step][(channel*W) +: W]),
                            $signed(threshold_flat[(channel*W) +: W]));
                end
            end

            generated_observation = 3'd0;
            for (channel = 0; channel < 3; channel = channel + 1) begin
                measurement_element = $signed(measurement_mem[step][(channel*W) +: W]);
                threshold_element = $signed(threshold_flat[(channel*W) +: W]);
            ...
            if (debug_cov_predict_flat !== cov_predict_expected_mem[step]) begin
                for (matrix_element = 0; matrix_element < 9; matrix_element = matrix_element + 1) begin
                    if ($signed(debug_cov_predict_flat[(matrix_element*W) +: W]) !==
                        $signed(cov_predict_expected_mem[step][(matrix_element*W) +: W]))
                        fail_mismatch(step, "cov_predict", matrix_element,
                            $signed(cov_predict_expected_mem[step][(matrix_element*W) +: W]),
                            $signed(debug_cov_predict_flat[(matrix_element*W) +: W]));
                end
            end
            if (debug_sign_cov_flat !== sign_cov_expected_mem[step]) begin
                for (matrix_element = 0; matrix_element < 9; matrix_element = matrix_element + 1) begin
                    if ($signed(debug_sign_cov_flat[(matrix_element*W) +: W]) !==
                        $signed(sign_cov_expected_mem[step][(matrix_element*W) +: W]))
                        fail_mismatch(step, "sign_cov", matrix_element,
                            $signed(sign_cov_expected_mem[step][(matrix_element*W) +: W]),
                            $signed(debug_sign_cov_flat[(matrix_element*W) +: W]));
                end
            end
            ...
            if (debug_gain_flat !== gain_expected_mem[step]) begin
                for (matrix_element = 0; matrix_element < 9; matrix_element = matrix_element + 1) begin
                    if ($signed(debug_gain_flat[(matrix_element*W) +: W]) !==
                        $signed(gain_expected_mem[step][(matrix_element*W) +: W]))
                        fail_mismatch(step, "gain", matrix_element,
                            $signed(gain_expected_mem[step][(matrix_element*W) +: W]),
                            $signed(debug_gain_flat[(matrix_element*W) +: W]));
                end
            end
            if (state_out_flat !== state_expected_mem[step]) begin
                for (channel = 0; channel < 3; channel = channel + 1) begin
                    if ($signed(state_out_flat[(channel*W) +: W]) !==
                        $signed(state_expected_mem[step][(channel*W) +: W]))
                        fail_mismatch(step, "state_post", channel,
                            $signed(state_expected_mem[step][(channel*W) +: W]),
                            $signed(state_out_flat[(channel*W) +: W]));
                end
            end
            if (cov_out_flat !== cov_expected_mem[step]) begin
                for (matrix_element = 0; matrix_element < 9; matrix_element = matrix_element + 1) begin
                    if ($signed(cov_out_flat[(matrix_element*W) +: W]) !==
                        $signed(cov_expected_mem[step][(matrix_element*W) +: W]))
                        fail_mismatch(step, "cov_post", matrix_element,
                            $signed(cov_expected_mem[step][(matrix_element*W) +: W]),
                            $signed(cov_out_flat[(matrix_element*W) +: W]));
                end
            end
            ...
            $fdisplay(rtl_file,
                ...
                $signed(threshold_flat[W-1:0]), $signed(threshold_flat[2*W-1:W]), $signed(threshold_flat[3*W-1:2*W]),
                generated_observation[0] ? 1 : -1,
                generated_observation[1] ? 1 : -1,
                generated_observation[2] ? 1 : -1,
                $signed(state_out_flat[W-1:0]), $signed(state_out_flat[2*W-1:W]), $signed(state_out_flat[3*W-1:2*W]),
                $signed(cov_out_flat[W-1:0]), $signed(cov_out_flat[2*W-1:W]), $signed(cov_out_flat[3*W-1:2*W]),
                $signed(cov_out_flat[4*W-1:3*W]), $signed(cov_out_flat[5*W-1:4*W]), $signed(cov_out_flat[6*W-1:5*W]),
                $signed(cov_out_flat[7*W-1:6*W]), $signed(cov_out_flat[8*W-1:7*W]), $signed(cov_out_flat[9*W-1:8*W]),

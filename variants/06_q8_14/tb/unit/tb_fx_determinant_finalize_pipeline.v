...
module tb_fx_determinant_finalize_pipeline;
    ...
    reg request_valid;
    wire request_ready;
    reg signed [2*W+1:0] request_accumulator;
    wire result_valid;
    reg result_ready;
    wire signed [W-1:0] result_determinant;
    ...
    reg signed [W-1:0] held_determinant;
    ...
    task run_matrix_case;
        input signed [W-1:0] matrix0;
        input signed [W-1:0] matrix1;
        input signed [W-1:0] matrix2;
        input signed [W-1:0] matrix3;
        input signed [W-1:0] matrix4;
        input signed [W-1:0] matrix5;
        input signed [W-1:0] matrix6;
        input signed [W-1:0] matrix7;
        input signed [W-1:0] matrix8;
        input signed [W-1:0] expected_determinant;
        input expected_solver_error;
        input expected_overflow;
        input [8*40-1:0] case_name;
        reg signed [2*W-1:0] minor_product_a;
        reg signed [2*W-1:0] minor_product_b;
        reg signed [W-1:0] cofactor0;
        reg signed [W-1:0] cofactor1;
        reg signed [W-1:0] cofactor2;
        reg signed [2*W-1:0] determinant_product0;
        reg signed [2*W-1:0] determinant_product1;
        reg signed [2*W-1:0] determinant_product2;
        reg signed [2*W+1:0] determinant_accumulator;
        begin
            ...
            determinant_accumulator = {{2{determinant_product0[2*W-1]}},
                                       determinant_product0};
            determinant_accumulator = determinant_accumulator +
                                      {{2{determinant_product1[2*W-1]}},
                                       determinant_product1};
            determinant_accumulator = determinant_accumulator +
                                      {{2{determinant_product2[2*W-1]}},
    ...
    task run_case;
        input signed [2*W+1:0] accumulator;
        input signed [W-1:0] expected_determinant;
        input expected_solver_error;
        input expected_overflow;
    ...
    task test_reset_flush;
        ...
        begin
            while (!request_ready) @(negedge clk);
            request_accumulator = (64'sd100*TB_ONE);
            request_valid = 1'b1;
            @(negedge clk);
            request_valid = 1'b0;
    ...
    task test_backpressure_and_rejected_request;
        ...
        begin
            ...
            request_accumulator = -(64'sd200*TB_ONE);
            request_valid = 1'b1;
            @(negedge clk);
            request_valid = 1'b0;
            ...
            check_result(100, 1'b0, 1'b0, "backpressure");
            ...
            run_case(-(64'sd200*TB_ONE), -200, 1'b0, 1'b0, 1,
                     "accepted_after_busy");
        end
    endtask
    ...
    initial begin
        ...
        rst_n = 1'b0;
        request_valid = 1'b0;
        request_accumulator = {(2*W+2){1'b0}};
        result_ready = 1'b1;
        cycle_count = 0;
        accepted_count = 0;
        completed_count = 0;
        ...
        run_matrix_case(
            fx_q16(64'sd131072), 0, 0,
            0, fx_q16(64'sd196608), 0,
            0, 0, -fx_q16(64'sd65536),
            -fx_q16(64'sd393216), 1'b0, 1'b0, "signed_diagonal_matrix");

        // Near-zero, strict floor, and rounding boundaries.
        run_case(-(TB_ONE/4), `FX_Q8_16_DET_FLOOR, 1'b1, 1'b0, 0, "negative_rounds_to_zero");
        run_case(((`FX_Q8_16_DET_FLOOR-1)*TB_ONE), `FX_Q8_16_DET_FLOOR, 1'b1, 1'b0, 0, "positive_near_floor");
        run_case(-((`FX_Q8_16_DET_FLOOR-1)*TB_ONE), -`FX_Q8_16_DET_FLOOR, 1'b1, 1'b0, 0, "negative_near_floor");
        run_case((`FX_Q8_16_DET_FLOOR*TB_ONE), `FX_Q8_16_DET_FLOOR, 1'b0, 1'b0, 0, "positive_exact_floor");
        run_case(-(`FX_Q8_16_DET_FLOOR*TB_ONE), -`FX_Q8_16_DET_FLOOR, 1'b0, 1'b0, 0, "negative_exact_floor");
        run_case((64'sd100*TB_ONE + TB_HALF-1), 100, 1'b0, 1'b0, 0, "positive_below_half");
        run_case((64'sd100*TB_ONE + TB_HALF), 101, 1'b0, 1'b0, 0, "positive_exact_half");
        run_case(-(64'sd100*TB_ONE + TB_HALF-1), -100, 1'b0, 1'b0, 0, "negative_below_half");
        run_case(-(64'sd100*TB_ONE + TB_HALF), -101, 1'b0, 1'b0, 0, "negative_exact_half");

        // Exact Q8.16 endpoints, saturation boundaries, and full 50-bit limits.
        run_case((TB_MAX*TB_ONE), TB_MAX_WORD, 1'b0, 1'b0, 0, "maximum_q8_16");
        run_case(-((-TB_MIN)*TB_ONE), TB_MIN_WORD, 1'b0, 1'b0, 0, "minimum_q8_16");
        run_case((TB_MAX*TB_ONE + TB_HALF), TB_MAX_WORD, 1'b0, 1'b1, 0, "positive_saturation");
        run_case(-((-TB_MIN)*TB_ONE + TB_HALF), TB_MIN_WORD, 1'b0, 1'b1, 0, "negative_saturation");
        run_case(((64'sd1 << (2*W+1))-1), TB_MAX_WORD, 1'b0, 1'b1, 0, "maximum_accumulator");
        run_case((-(64'sd1 << (2*W+1))), TB_MIN_WORD, 1'b0, 1'b1, 0, "minimum_accumulator");

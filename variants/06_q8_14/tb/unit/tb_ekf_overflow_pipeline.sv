...
module tb_ekf_overflow_pipeline;
    ...
    localparam signed [63:0] FX_MAX_RAW = (TB_MAX*TB_ONE);
    localparam signed [63:0] FX_MIN_RAW = (TB_MIN*TB_ONE);
    ...
    reg signed [2*W+1:0] random_accumulator;
    reg signed [63:0] random_wide;
    reg signed [W-1:0] captured_rounded;
    ...
    function automatic signed [W-1:0] reference_round_sat50;
        ...
        begin
            ...
            if (extended_value < 0)
                ...
                rounded_value = (extended_value + TB_HALF) >>> F;
            if (rounded_value > TB_MAX)
                reference_round_sat50 = TB_MAX_WORD;
            else if (rounded_value < -(-TB_MIN))
                reference_round_sat50 = TB_MIN_WORD;
            else
                reference_round_sat50 = rounded_value[W-1:0];
        end
    endfunction
    ...
    function automatic reference_overflow50;
        ...
        begin
            ...
            if (extended_value < 0)
                ...
                rounded_value = (extended_value + TB_HALF) >>> F;
            reference_overflow50 = (rounded_value > TB_MAX) ||
    ...
    task automatic inject_and_record;
        ...
        input sticky_before;
        reg signed [W-1:0] expected_rounded;
        reg expected_overflow;
        reg expected_sticky;
        reg signed [W-1:0] observed_rounded;
        reg observed_sticky;
    ...
    task automatic run_sticky_sequence;
        ...
        reg sticky_expected;
        reg signed [2*W+1:0] sequence_accumulator;
        begin
            clear_transaction_status();
            ...
            for (sequence_index = 0; sequence_index < 9;
                ...
                if ((overflow_pattern & (1 << sequence_index)) != 0)
                    sequence_accumulator = FX_MAX_RAW + TB_ONE;
                else
                    sequence_accumulator = (sequence_index - 4) * TB_ONE;
    ...
    task automatic run_busy_start_ignored;
        begin
            clear_transaction_status();
            f_input_flat = {9{fx_q16(64'sh012345)}};
            @(negedge clk);
            dut.state = ST_ADD_Q;
            model_valid = 1'b1;
            #1;
    ...
    initial begin
        ...
        run_independent("directed", "zero", {(2*W+2){1'b0}}, 0);
        run_independent("directed", "positive_one_raw", {{(2*W+1){1'b0}},1'b1}, 1);
        run_independent("directed", "negative_one_raw", -{{(2*W+1){1'b0}},1'b1}, 2);
        ...
        run_independent("directed", "min_minus_one_output_lsb",
                        FX_MIN_RAW - TB_ONE, 8);
        run_independent("directed", "positive_round_zero_below", (TB_HALF-1), 0);
        run_independent("directed", "positive_round_zero_cutoff", TB_HALF, 1);
        run_independent("directed", "positive_round_zero_above", (TB_HALF+1), 2);
        run_independent("directed", "negative_round_zero_above", -(TB_HALF-1), 3);
        run_independent("directed", "negative_round_zero_cutoff", -TB_HALF, 4);
        run_independent("directed", "negative_round_zero_below", -(TB_HALF+1), 5);
        run_independent("directed", "positive_overflow_cutoff_below",
                        FX_MAX_RAW + (TB_HALF-1), 6);
        run_independent("directed", "positive_overflow_cutoff",
                        FX_MAX_RAW + TB_HALF, 7);
        run_independent("directed", "positive_overflow_cutoff_above",
                        FX_MAX_RAW + (TB_HALF+1), 8);
        run_independent("directed", "negative_overflow_cutoff_above",
                        FX_MIN_RAW - (TB_HALF-1), 0);
        run_independent("directed", "negative_overflow_cutoff",
                        FX_MIN_RAW - TB_HALF, 1);
        run_independent("directed", "negative_overflow_cutoff_below",
                        FX_MIN_RAW - (TB_HALF+1), 2);
        directed_count = 21;
        ...
        for (case_index = 0; case_index < RANDOM_CASES;
            ...
            case (case_index % 4)
                0: begin
                    random_accumulator = {random_high[2*W-31:0], random_low};
                end
                1: begin
                    offset_value = random_low % (2*TB_ONE+1);
                    offset_value = offset_value - TB_ONE;
                    random_wide = FX_MAX_RAW + offset_value;
                    random_accumulator = random_wide[2*W+1:0];
                end
                2: begin
                    offset_value = random_low % (2*TB_ONE+1);
                    offset_value = offset_value - TB_ONE;
                ...
                default: begin
                    random_wide = $signed(random_low[W-1:0]);
                    random_wide = (random_wide * TB_ONE) +
                                  $signed({1'b0, random_high[F-2:0]});
                    random_accumulator = random_wide[2*W+1:0];
                end
            endcase
    ...
    initial begin
        #20000000;
        $fatal(1, "FAIL: EKF overflow regression timeout transactions=%0d", transaction_count);
    end
endmodule

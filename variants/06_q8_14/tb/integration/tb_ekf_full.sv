...
module tb_ekf_full;
    ...
    wire [9*W-1:0] debug_innovation_cov_flat;
    wire [9*W-1:0] debug_gain_flat;
    wire [3*W-1:0] debug_innovation_flat;
    wire signed [W-1:0] debug_determinant;

    reg [3*W-1:0] init_state [0:0];
    reg [9*W-1:0] init_cov [0:0];
    reg [9*W-1:0] f_mem [0:STEPS-1];
    reg [3*W-1:0] measurement_mem [0:STEPS-1];
    reg [9*W-1:0] cov_inner_mem [0:STEPS-1];
    reg [9*W-1:0] cov_predict_mem [0:STEPS-1];
    reg [9*W-1:0] innovation_cov_mem [0:STEPS-1];
    reg [9*W-1:0] gain_mem [0:STEPS-1];
    reg [W-1:0] determinant_mem [0:STEPS-1];
    reg [3*W-1:0] state_mem [0:STEPS-1];
    reg [9*W-1:0] cov_mem [0:STEPS-1];
    integer step;
...
`ifdef WNS_CLOSURE_ASSERTIONS
    ...
    reg cov_predict_mac_valid_d = 1'b0;
    reg signed [2*W+1:0] cov_predict_mac_accumulator_d = {(2*W+2){1'b0}};
    reg cov_predict_mac_last_d = 1'b0;
    reg [5:0] cov_predict_mac_state_d = 6'd0;
    reg cov_predict_overflow_local_valid_d = 1'b0;
    reg cov_predict_overflow_local_candidate_d = 1'b0;
    reg cov_predict_overflow_local_last_d = 1'b0;
    reg cov_predict_last_local_seen = 1'b0;
    reg cov_predict_last_overflow_expected = 1'b0;

    function automatic overflow50_reference;
        input signed [2*W+1:0] value;
        reg signed [2*W+2:0] extended_value;
        reg signed [2*W+2:0] rounded_value;
        begin
            extended_value = {value[2*W+1], value};
            if (extended_value < 0)
                rounded_value = -(((-extended_value) + TB_HALF) >>> F);
            else
                rounded_value = (extended_value + TB_HALF) >>> F;
            overflow50_reference = (rounded_value > TB_MAX) ||
                                   (rounded_value < -(-TB_MIN));
        end
    endfunction
...
`ifdef WNS_CLOSURE_ASSERTIONS
            ...
            cov_predict_mac_valid_d <= 1'b0;
            cov_predict_mac_accumulator_d <= {(2*W+2){1'b0}};
            cov_predict_mac_last_d <= 1'b0;
            cov_predict_mac_state_d <= 6'd0;
            cov_predict_overflow_local_valid_d <= 1'b0;
            cov_predict_overflow_local_candidate_d <= 1'b0;
            cov_predict_overflow_local_last_d <= 1'b0;
            cov_predict_last_local_seen <= 1'b0;
            cov_predict_last_overflow_expected <= 1'b0;
            if (dut.u_engine.cov_predict_mac_valid || dut.u_engine.add_q_operand_valid ||
                ...
                dut.u_engine.cov_predict_overflow_local_valid_reg ||
                dut.u_engine.cov_predict_overflow_local_reg ||
...
`ifdef WNS_CLOSURE_ASSERTIONS
            ...
            if (dut.u_engine.cov_predict_overflow_local_valid_reg !==
                cov_predict_mac_valid_d) begin
                $display("FAIL: EKF overflow local valid is not delayed cov_predict_mac_valid");
                $fatal(1);
            end
            if (dut.u_engine.cov_predict_overflow_local_valid_reg) begin
                if (cov_predict_mac_state_d !== dut.u_engine.ST_COV_OUTER_COMMIT) begin
                    $display("FAIL: EKF overflow local valid did not originate in covariance commit flow");
                    $fatal(1);
                end
                if (dut.u_engine.cov_predict_overflow_local_reg !==
                    overflow50_reference(cov_predict_mac_accumulator_d)) begin
                    $display("FAIL: EKF overflow local candidate differs from overflow50 semantics");
                    $fatal(1);
                end
                if (cov_predict_mac_last_d) begin
                    if (dut.u_engine.state !== dut.u_engine.ST_ADD_Q) begin
                        $display("FAIL: EKF last covariance overflow candidate is not aligned with downstream flow");
                        $fatal(1);
                    end
                    cov_predict_last_local_seen <= 1'b1;
                    if (dut.u_engine.cov_predict_overflow_local_reg)
                        cov_predict_last_overflow_expected <= 1'b1;
                end else if (dut.u_engine.state !== dut.u_engine.ST_COV_OUTER) begin
                    $display("FAIL: EKF non-final overflow candidate is not aligned with covariance flow");
                    $fatal(1);
                end
            end
            if (cov_predict_overflow_local_valid_d &&
                cov_predict_overflow_local_candidate_d &&
                (overflow_flag !== 1'b1)) begin
                $display("FAIL: EKF registered overflow candidate did not commit to sticky flag");
                $fatal(1);
            end
            if (cov_predict_overflow_local_last_d &&
                cov_predict_overflow_local_candidate_d &&
                (overflow_flag !== 1'b1)) begin
                $display("FAIL: EKF final covariance element overflow was lost");
                $fatal(1);
            end
            if (result_valid || done) begin
                if (dut.u_engine.cov_predict_mac_valid || cov_predict_mac_valid_d ||
                    dut.u_engine.cov_predict_overflow_local_valid_reg ||
                    cov_predict_overflow_local_valid_d) begin
                    $display("FAIL: EKF result/done asserted with covariance overflow work pending");
                    $fatal(1);
                end
                if (!cov_predict_last_local_seen) begin
                    $display("FAIL: EKF result/done preceded final covariance overflow candidate");
                    $fatal(1);
                end
                if (cov_predict_last_overflow_expected &&
                    (overflow_flag !== 1'b1)) begin
                    $display("FAIL: EKF final covariance overflow was not sticky through result/done");
                    $fatal(1);
                end
            end
            if ((^{dut.u_engine.cov_predict_mac_valid, dut.u_engine.add_q_operand_valid,
            ...
            cov_predict_mac_valid_d <= dut.u_engine.cov_predict_mac_valid;
            cov_predict_mac_accumulator_d <=
                dut.u_engine.cov_predict_mac_accumulator_reg;
            cov_predict_mac_last_d <= dut.u_engine.cov_predict_mac_last_reg;
            cov_predict_mac_state_d <= dut.u_engine.state;
            cov_predict_overflow_local_valid_d <=
                dut.u_engine.cov_predict_overflow_local_valid_reg;
            cov_predict_overflow_local_candidate_d <=
                dut.u_engine.cov_predict_overflow_local_reg;
            cov_predict_overflow_local_last_d <=
                dut.u_engine.cov_predict_overflow_local_valid_reg &&
                cov_predict_mac_last_d;
`endif
        end
    end
    ...
    initial begin
        ...
        $fdisplay(rtl_file, "step,state_0_int,state_1_int,state_2_int,cov_00_int,cov_01_int,cov_02_int,cov_10_int,cov_11_int,cov_12_int,cov_20_int,cov_21_int,cov_22_int,overflow,numeric_error,solver_error");
        ...
        for (step = 0; step < requested_steps; step = step + 1) begin
            ...
            for (matrix_check_index = 0; matrix_check_index < 9;
                ...
                if ($signed(dut.u_engine.f_matrix[matrix_check_index]) !==
                    $signed(f_mem[step][(matrix_check_index*W) +: W]))
                    fail("f_matrix_capture");
            end
            while (!result_valid) @(negedge clk);
            if ((^{overflow_flag, numeric_error, solver_error}) === 1'bx)
                $fatal(1, "FAIL: status contains X/Z step=%0d", step);
            ...
            for (matrix_check_index = 0; matrix_check_index < 9;
                ...
                if ($signed(dut.u_engine.matrix_temp[matrix_check_index]) !==
                    $signed(cov_inner_mem[step][(matrix_check_index*W) +: W]))
            ...
            $fdisplay(rtl_file, "%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d",
                step, $signed(state_out_flat[W-1:0]), $signed(state_out_flat[2*W-1:W]),
                $signed(state_out_flat[3*W-1:2*W]), $signed(cov_out_flat[W-1:0]),
                $signed(cov_out_flat[2*W-1:W]), $signed(cov_out_flat[3*W-1:2*W]),
                $signed(cov_out_flat[4*W-1:3*W]), $signed(cov_out_flat[5*W-1:4*W]),
                $signed(cov_out_flat[6*W-1:5*W]), $signed(cov_out_flat[7*W-1:6*W]),
                $signed(cov_out_flat[8*W-1:7*W]), $signed(cov_out_flat[9*W-1:8*W]), overflow_flag, numeric_error, solver_error);

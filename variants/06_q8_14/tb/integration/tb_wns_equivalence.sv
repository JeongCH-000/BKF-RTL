...
module tb_wns_equivalence #(
    ...
    wire [3*W-1:0] state_post_trace;
    wire [9*W-1:0] cov_post_trace;
    wire [9*W-1:0] f_matrix_trace;
    wire [9*W-1:0] cov_inner_trace;
    wire [9*W-1:0] cov_predict_trace;
    wire [9*W-1:0] measurement_cov_trace;
    wire [9*W-1:0] sign_cov_trace;
    wire [9*W-1:0] gain_trace;
    wire [3*W-1:0] observation_q_trace;
    ...
    generate
        for (vector_index = 0; vector_index < 3; vector_index = vector_index + 1) begin : PACK_VECTOR_TRACE
            assign state_post_trace[(vector_index*W) +: W] = dut.state_post[vector_index];
            assign observation_q_trace[(vector_index*W) +: W] = dut.observation_q[vector_index];
        end
    endgenerate

    // EKF has no branch observation transaction. For BKF/rBKF the input bits
    // remain stable through result_valid; capture their signed integer sums.
    ...
    generate
        for (matrix_index = 0; matrix_index < 9; matrix_index = matrix_index + 1) begin : PACK_MATRIX_TRACE
            assign cov_post_trace[(matrix_index*W) +: W] = dut.cov_post[matrix_index];
            assign f_matrix_trace[(matrix_index*W) +: W] = dut.f_matrix[matrix_index];
            assign cov_inner_trace[(matrix_index*W) +: W] = dut.matrix_temp[matrix_index];
            assign cov_predict_trace[(matrix_index*W) +: W] = dut.cov_predict[matrix_index];
            assign measurement_cov_trace[(matrix_index*W) +: W] = dut.measurement_cov[matrix_index];
            assign sign_cov_trace[(matrix_index*W) +: W] = dut.sign_cov[matrix_index];
            assign gain_trace[(matrix_index*W) +: W] = dut.gain[matrix_index];
        end
    endgenerate
    ...
    initial begin
        ...
        for (step = 0; step < requested_steps; step = step + 1) begin
            ...
            $fdisplay(
                trace_file,
                "%0d,%0h,%0h,%0h,%0h,%0h,%0h,%0h,%0h,%0h,%0h,%0h,%0h,%0d,%0d,%0d,%0d,%0d,%0d",
                step,
                state_post_trace,
                cov_post_trace,
                f_matrix_trace,

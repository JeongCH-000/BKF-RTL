...
from nominal_config import Q_FORMAT, FRAC, validate_generated_header, validate_reference_format
...
def main() -> None:
    ...
    if not (reference / "stimulus.npz").is_file():
    ...
    metadata = {
        ...
        "seed": SEED,
        "steps": STEPS,
        "branch_order": "branch-major: flattened index = branch*3 + feature",
        "matrix_order": f"row-major, element zero in least-significant {WIDTH}-bit slice",
        "q_format": Q_FORMAT,
        "signed_width": WIDTH,
        "fractional_bits": FRAC,

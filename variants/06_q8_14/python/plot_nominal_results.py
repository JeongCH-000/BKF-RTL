...
def load_rtl(name: str) -> np.ndarray | None:
...
def main() -> None:
    ...
    rtl = {name: load_rtl(name) for name in ALGORITHMS}
    ...
    for row, name in enumerate(ALGORITHMS):
        for state in range(3):
            ...
            if rtl[name] is not None:
                count = len(rtl[name])
                label = "RTL" if count == len(target) else f"RTL ({count} completed steps)"
                axis.plot(np.arange(count), rtl[name][:, state], color="#CC79A7", linestyle="--", linewidth=0.7, label=label)
            if state == 0:

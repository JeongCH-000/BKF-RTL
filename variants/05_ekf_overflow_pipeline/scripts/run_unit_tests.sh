...
mkdir -p results/rtl \
  ...
  results/regression/ekf_overflow_pipeline
...
run_wns_equivalence_suite reference ../04_wns_closure
...
echo "PASS: WNS equivalence reference=04_wns_closure current=05_ekf_overflow_pipeline configs=4 fields=19 steps=500 mismatches=0"

reference_overflow_csv=results/regression/ekf_overflow_pipeline/reference_transactions.csv
current_overflow_csv=results/regression/ekf_overflow_pipeline/current_transactions.csv

iverilog -g2012 -gstrict-expr-width -Wall -Wno-sensitivity-entire-array -Wimplicit \
  -I../04_wns_closure -I. \
  -s tb_ekf_overflow_pipeline \
  -o results/rtl/tb_ekf_overflow_pipeline_reference.vvp \
  ../04_wns_closure/rtl/common/fx_divider_q8_16.v \
  ../04_wns_closure/rtl/common/fx_determinant_finalize_pipeline.v \
  ../04_wns_closure/rtl/common/fx_mul_mac_pipeline.v \
  ../04_wns_closure/rtl/nonlinear/q8_16_rsqrt_lut.v \
  ../04_wns_closure/rtl/nonlinear/arcsine_cov_lut_q8_16.v \
  ../04_wns_closure/rtl/bkf/bkf_core.v \
  tb/unit/tb_ekf_overflow_pipeline.sv
vvp results/rtl/tb_ekf_overflow_pipeline_reference.vvp \
  "+OUTPUT=$reference_overflow_csv"

iverilog -g2012 -gstrict-expr-width -Wall -Wno-sensitivity-entire-array -Wimplicit -I. \
  -DEKF_OVERFLOW_PIPELINED \
  -s tb_ekf_overflow_pipeline \
  -o results/rtl/tb_ekf_overflow_pipeline_current.vvp \
  rtl/common/fx_divider_q8_16.v \
  rtl/common/fx_determinant_finalize_pipeline.v \
  rtl/common/fx_mul_mac_pipeline.v \
  rtl/nonlinear/q8_16_rsqrt_lut.v \
  ...
  rtl/bkf/bkf_core.v \
  tb/unit/tb_ekf_overflow_pipeline.sv
vvp results/rtl/tb_ekf_overflow_pipeline_current.vvp \
  "+OUTPUT=$current_overflow_csv"

for trace in "$reference_overflow_csv" "$current_overflow_csv"; do
  if ! awk -F, 'NF != 14 { bad = 1 } END { exit (bad || NR != 2074) }' "$trace"; then
    echo "FAIL: malformed EKF overflow trace: $trace" >&2
    exit 1
  fi
done

cut -d, -f1-12 "$reference_overflow_csv" \
  > results/regression/ekf_overflow_pipeline/reference_semantics.csv
cut -d, -f1-12 "$current_overflow_csv" \
  > results/regression/ekf_overflow_pipeline/current_semantics.csv
if ! cmp \
  results/regression/ekf_overflow_pipeline/reference_semantics.csv \
  results/regression/ekf_overflow_pipeline/current_semantics.csv; then
  echo "FAIL: EKF overflow semantic mismatch between 04 and 05" >&2
  exit 1
fi

echo "PASS: EKF overflow reference=04_wns_closure current=05_ekf_overflow_pipeline transactions=2073 semantic_mismatches=0"

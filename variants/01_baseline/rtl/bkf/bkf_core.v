...
module bkf_core #(
    ...
    wire signed [47:0] multiply_product;
    wire signed [49:0] product_extended;
    wire signed [49:0] mac_with_product;
    wire signed [23:0] rounded_product;
    wire signed [23:0] rounded_mac;
    ...
    assign multiply_product = $signed(mul_a) * $signed(mul_b);
    assign product_extended = {{2{multiply_product[47]}}, multiply_product};
    assign mac_with_product = mac_accumulator + product_extended;
    assign rounded_product = round_sat48(multiply_product);
    assign rounded_mac = round_sat50(mac_with_product);
    assign product_overflow = overflow48(multiply_product);
    assign mac_overflow = overflow50(mac_with_product);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                ST_XPRED: begin
                    if (dot_index == 2'd0) begin
                        mac_accumulator <= product_extended;
                        dot_index <= 2'd1;
                    end else if (dot_index == 2'd1) begin
                        mac_accumulator <= mac_with_product;
                        dot_index <= 2'd2;
                    end else begin
                        state_predict[row_index] <= rounded_mac;
                        if (mac_overflow) overflow_flag <= 1'b1;
                        dot_index <= 2'd0;
                        mac_accumulator <= 50'sd0;
                ...
                ST_COV_INNER, ST_COV_OUTER, ST_GAIN, ST_COV_UP_INNER, ST_COV_CORR: begin
                    if (dot_index == 2'd0) begin
                        ...
                        if (state == ST_COV_INNER)
                            matrix_temp[(row_index*3)+col_index] <= rounded_mac;
                        else if (state == ST_COV_OUTER)
                            cov_predict[(row_index*3)+col_index] <= rounded_mac;
                        else if (state == ST_GAIN)
                            gain[(row_index*3)+col_index] <= rounded_mac;
                        else if (state == ST_COV_UP_INNER)
                            cov_update_inner[(row_index*3)+col_index] <= rounded_mac;
                        else
                            cov_correction[(row_index*3)+col_index] <= rounded_mac;
                        if (mac_overflow) overflow_flag <= 1'b1;
                        dot_index <= 2'd0;
                        mac_accumulator <= 50'sd0;
                        if (col_index == 2'd2) begin
                            col_index <= 2'd0;
                            if (row_index == 2'd2) begin
                                row_index <= 2'd0;
                                if (state == ST_COV_INNER)
                ...
                ST_PNORM_M1: begin
                    if ((element_index == 4'd0) || (element_index == 4'd4) || (element_index == 4'd8)) begin
                        ...
                        scalar_temp <= rounded_product;
                        if (product_overflow) overflow_flag <= 1'b1;
                        state <= ST_PNORM_M2;
                    end
                end

                ST_PNORM_M2: begin
                    if ($signed(rounded_product) > $signed(FX_ONE)) begin
                        normalized_cov[element_index] <= FX_ONE;
                        numeric_error <= 1'b1;
                    end else if ($signed(rounded_product) < $signed(FX_NEG_ONE)) begin
                        normalized_cov[element_index] <= FX_NEG_ONE;
                        numeric_error <= 1'b1;
                    end else begin
                        normalized_cov[element_index] <= rounded_product;
                    end
                    if (product_overflow) overflow_flag <= 1'b1;
                    if (element_index == 4'd8) begin
                        element_index <= 4'd0;
                ...
                ST_SELF_M2: begin
                    self_normalized[element_index] <= rounded_product;
                    if (product_overflow) overflow_flag <= 1'b1;
                ...
                ST_B_CALC: begin
                    b_diag[element_index] <= rounded_product;
                    if (product_overflow) overflow_flag <= 1'b1;
                    if (element_index == 4'd2) begin
                        element_index <= 4'd0;
                        state <= ST_COF_P1;
                ...
                ST_COF_P2: begin
                    cofactor[element_index] <= sub_sat24(scalar_temp, rounded_product);
                    if (product_overflow || sub_overflow24(scalar_temp, rounded_product)) overflow_flag <= 1'b1;
                    if (element_index == 4'd8) begin
                        element_index <= 4'd0;
                        dot_index <= 2'd0;
                        mac_accumulator <= 50'sd0;
                ...
                ST_DET: begin
                    if (dot_index == 2'd0) begin
                        ...
                        if (($signed(rounded_mac) < $signed(DET_FLOOR)) &&
                            ($signed(rounded_mac) > -$signed(DET_FLOOR))) begin
                            determinant <= rounded_mac[23] ? -DET_FLOOR : DET_FLOOR;
                            solver_error <= 1'b1;
                        end else begin
                            determinant <= rounded_mac;
                        end
                        if (mac_overflow) overflow_flag <= 1'b1;
                        element_index <= 4'd0;
                        dot_index <= 2'd0;
                ...
                ST_GAIN_INNER: begin
                    gain_inner[element_index] <= rounded_product;
                    if (product_overflow) overflow_flag <= 1'b1;
                    if (element_index == 4'd8) begin
                        row_index <= 2'd0;
                        col_index <= 2'd0;
                ...
                ST_X_MV: begin
                    if (dot_index == 2'd0) begin
                        ...
                        state_correction[row_index] <= rounded_mac;
                        if (mac_overflow) overflow_flag <= 1'b1;
                        dot_index <= 2'd0;
                        mac_accumulator <= 50'sd0;
                        if (row_index == 2'd2) begin
